...
def get_collection_from_db(session: Session, season_id: int) -> List[Tuple[Collection, Video]]:
    statement = select(Collection, Video).join(Video, Collection.season_id == Video.collection_id).where(
        Collection.season_id == season_id).order_by(Video.order_index)
...
def get_all_collections_from_db(session: Session) -> List[Tuple[Collection, Video]]:
    statement = select(Collection, Video).join(Video, Collection.season_id == Video.collection_id).order_by(
        Collection.order_index, Video.order_index)
    results = session.exec(statement).all()
    return results
...
# 更新合集信息
def update_collection_to_db(session: Session, season_id: int) -> Tuple[dict, str, datetime, int]:
    # 查出合集中的视频
